...
| 1.7 | PRD §3.1.2 *Admin Policy Auto-Seeding* | silent-missing | major | PRD: "When a new module is added… its migration seeds `level: 'full'` policies for the `admin` role in every existing tenant schema." No per-module retroactive seeder exists. `systemRoleSeeder.js:66` shows the admin role uses a single `module: ''` wildcard `level: 'full'` grant — so admin works in practice, but the documented mechanism (per-module migration that seeds across existing tenants) is not what is implemented. Documentation describes a different design than the code. | §3.1.2 | unmarked |
