...
Feature modules live in `apps/server/src/modules/` (lowercase), separate from core platform code in `src/system/`. Each module is self-contained with its own schemas, models, controllers, routers, and migrations. Modules import platform code via relative paths (e.g., `../../../lib/BaseController.js`). Cross-boundary service imports use barrel `services/index.js` files for a stable API surface (see ADR-0019).
