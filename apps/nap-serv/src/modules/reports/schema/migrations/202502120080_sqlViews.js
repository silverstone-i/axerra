  ...
  async up({ schema, db, pgp }) {
    ...
    await db.none(`
      ...
      SELECT
        ...
        -- Outstanding AR
        COALESCE(ar_agg.invoiced_revenue, 0) - COALESCE(ar_agg.collected_revenue, 0)
          AS outstanding_ar,
        ...
        CASE
          WHEN COALESCE(p.contract_amount, 0) = 0 THEN 0
          ELSE ROUND(
            (p.contract_amount - (
              COALESCE(ac_agg.actual_spend, 0)
              + GREATEST(COALESCE(ci_agg.total_budgeted_cost, 0) - COALESCE(ac_agg.actual_spend, 0), 0)
            )) * 100.0 / p.contract_amount, 2
          )
    ...
    await db.none(`
      ...
      GROUP BY da.project_id, cat.id, cat.code, cat.name, cat.type
    `);
    ...
    await db.none(`
      ...
      SELECT
        ...
        SUM(ai.total_amount - COALESCE(r_agg.paid, 0)) AS total_balance,
        SUM(ai.total_amount - COALESCE(r_agg.paid, 0))
          FILTER (WHERE CURRENT_DATE - ai.due_date <= 0)                      AS current_bucket,
        SUM(ai.total_amount - COALESCE(r_agg.paid, 0))
          FILTER (WHERE CURRENT_DATE - ai.due_date BETWEEN 1 AND 30)          AS bucket_1_30,
        SUM(ai.total_amount - COALESCE(r_agg.paid, 0))
          FILTER (WHERE CURRENT_DATE - ai.due_date BETWEEN 31 AND 60)         AS bucket_31_60,
        SUM(ai.total_amount - COALESCE(r_agg.paid, 0))
      ...
      WHERE ai.deactivated_at IS NULL
        ...
        AND (ai.total_amount - COALESCE(r_agg.paid, 0)) > 0
      GROUP BY ai.client_id, cl.name, cl.code
    `);
    ...
    await db.none(`
      ...
      SELECT
        ...
        SUM(api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0)) AS total_balance,
        SUM(api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0))
          FILTER (WHERE CURRENT_DATE - api.due_date <= 0)                     AS current_bucket,
        SUM(api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0))
          FILTER (WHERE CURRENT_DATE - api.due_date BETWEEN 1 AND 30)         AS bucket_1_30,
        SUM(api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0))
          FILTER (WHERE CURRENT_DATE - api.due_date BETWEEN 31 AND 60)        AS bucket_31_60,
        SUM(api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0))
      ...
      WHERE api.deactivated_at IS NULL
        ...
        AND (api.total_amount - COALESCE(p_agg.paid, 0) - COALESCE(cm_agg.credited, 0)) > 0
