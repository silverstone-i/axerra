 ...
 * @module vimber-serv/services/rbacQueryContext
 *
 ...
 *
 * @param {object} perms Full permission canon from req.user.permissions
 * @param {string} module Module name (e.g., 'ar')
 * @param {string} router Router name (e.g., 'ar-invoices')
 * @returns {object} Query context: { scope, projectIds, companyIds, visibleStatuses, allowedColumns, entityType, entityId }
 */
export function buildQueryContext(perms, module, router) {
  ...
  return {
    ...
    // Entity info for self-scope filtering
    entityType: perms?.entityType || null,
    entityId: perms?.entityId || null,
