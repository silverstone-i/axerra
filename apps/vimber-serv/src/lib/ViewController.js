 ...
 * @module vimber-serv/lib/ViewController
 *
 ...
 *   { module: 'ar', router: 'ar-invoices', scopeColumn: 'project_id', entityScopeColumns: { vendor: 'vendor_id' } }
 *
...
import { buildQueryContext } from '../services/rbacQueryContext.js';
...
class ViewController {
  constructor(modelName, errorLabel = null) {
    ...
    /** @type {{ module: string, router: string, scopeColumn: string, entityScopeColumns: object } | null} */
   ...
   * No-op when rbacConfig is null.
   *
   * @param {object} req Express request (for permissions)
  ...
  _applyRbacFilters(req, conditions, options) {
    ...
    // Layer 2: Data scope
    if (qctx.scope === 'self') {
      // Filter by entity's own FK column
      const entityScopeColumns = this.rbacConfig.entityScopeColumns || {};
      const scopeCol = entityScopeColumns[qctx.entityType];
   ...
   */
  handleError(err, res, action, label) {
    const pgStatus = PG_ERROR_STATUS[err?.code];
    if (pgStatus) {
      return res.status(pgStatus).json({
        error: `Error ${action} ${label}`,
        detail: err.detail || err.message,
        constraint: err.constraint,
      });
    }

    logger.error(`Error ${action} ${label}`, { error: err?.message, stack: err?.stack });
