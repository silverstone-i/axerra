 ...
 *
 * Cost lines track individual cost entries per deliverable/activity.
 * The `amount` column is a PostgreSQL GENERATED ALWAYS AS
 * (quantity * unit_price) STORED column — added via ALTER TABLE
 * in migration to keep it out of the ColumnSet.
 *
...
const costLinesSchema = {
  ...
  columns: [
    ...
    // amount GENERATED ALWAYS AS (quantity * unit_price) STORED — added via ALTER TABLE
