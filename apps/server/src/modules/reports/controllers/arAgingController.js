...
class ArAgingController extends ReportController {
  async getAll(req, res) {
    try {
      ...
      const rows = await db.manyOrNone(`SELECT * FROM ${s}.vw_ar_aging ORDER BY client_name`);
      res.json(rows);
    } catch (err) {
  ...
  async getByClient(req, res) {
    try {
      ...
      const row = await db.oneOrNone(`SELECT * FROM ${s}.vw_ar_aging WHERE client_id = $1`, [clientId]);
