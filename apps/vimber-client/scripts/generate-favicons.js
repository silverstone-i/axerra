 ...
 * @module vimber-client/scripts/generate-favicons
 *
 ...
 *
 * Outputs land in both `apps/vimber-client/public/` (runtime) and
 * `docs/Branding/files/` (docs reference for the preview HTML).
 *
 * Run: `npm -w apps/vimber-client run favicons`
 *
 ...
 *   (≈0.19 for default sizes, lifted at 16px so the accent survives downsampling).
 */
...
async function main() {
  ...
  const defaultSvg = composeSvg({ dotRatio: 0.19 });
  const tinySvg = composeSvg({ dotRatio: 0.24 }); // 16px size-adaptive variant
